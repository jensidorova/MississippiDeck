...
# 
# #assets <- filterAssets(data = assets, fy = 2001, plan = "South Carolina Retirement Systems")
# #View(assets)
...
#Pull plan-specific data from the database
pl <- planList()
reason.data <- pullData(pl, "Public Employees' Retirement System of Mississippi") %>% filter(year > 2000 & !duplicated(year))# (2002-2020)
#reason.data <- pullData(pl, "Louisiana State Employees Retirement System")# (2002-2019)
#reason.data <- pullData(pl, "New Mexico Public Employees Retirement Association")# (2002-2019)
#reason.data <- pullData(pl, "Teachers’ Retirement System of Louisiana")# (2002-2019)
#reason.data <- pullData(pl, "Arizona Public Safety Personnel Retirement System") (2002-2019)
...
```{r payroll, dpi=400, echo = FALSE, fig.cap = "Source: Pension Integrity Project analysis of SCRS actuarial valuation reports.", fig.width=8, fig.height=5, fig.align = "center", message=FALSE, warning=FALSE}
pl <- planList()
#PULL DATA
###################
#Using 2 New Functions to download and filter datat from datatabase
...
Data <- pullStateData(2001)
Data <- filterData(Data, 2001)
Data<- Data %>% filter(plan_name == "Public Employees' Retirement System of Mississippi") %>% filter(!duplicated(year))
Data$year <- as.numeric(Data$year)
Data <- data.frame(Data)
...
reason.data$net.amo[1:3] <- c(0,0,0)
#Actual Positive Amo
...
pw.28 <- round(as.numeric(SalaryData2$PVPenWealth[28])/1000,2)
pw.32 <- round(as.numeric(SalaryData2$PVPenWealth[31])/1000,2)
...
pwealth <- ggplot(SalaryData2, aes(Age,PVPenWealth/1000))+
  ...
  scale_y_continuous(breaks = seq(0, 5000, by = 100),limits = c(0, y_max/1000*1.05), labels = function(x) paste0("$",x),
...
Only **$`r pw.28`K** is what new Age `r entry` (Class Three) SCRS members will accrue after 28 years on the job (value of future benefits at retirement) .
...
But this value spikes to **$`r pw.32`K** or **`r pw.32.p`%** jump after just 4 more years (long enough to receive Full Benefits)
