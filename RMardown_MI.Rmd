...
---
title: "Public Employees' Retirement System of Mississippi"
...
#View(Data)
Data <-
  data.table(Data) %>% filter(plan_name == "Public Employees' Retirement System of Mississippi")
...
Data$aal <- as.numeric(Data$aal)
Data$year <- as.numeric(Data$year)
Data <- Data %>% filter(plan_name == "Public Employees' Retirement System of Mississippi")
Data$unfunded_actuarially_accrued_liabilities_dollar <- as.numeric(Data$unfunded_actuarially_accrued_liabilities_dollar)
#Manually calculating 2021 AVA (AAL - UAL)
Data$ava[21] <- Data$aal[21] - Data$unfunded_actuarially_accrued_liabilities_dollar[21]
#Data<- Data %>% 
...
### Compound Net Change to UAL w/ R (barPlot())

```{r, dpi = 400, echo = FALSE, fig.cap = "Source: Pension Integrity Project analysis of SCRS actuarial valuation reports.", compoundGL,fig.width=8, fig.height=5, fig.align = "center", message=FALSE, warning=FALSE}
rm(list=ls())
library(reasontheme)
library(pensionviewr)
library(ggplot2)
...
filterAssets <- function (data, fy = 2001, plan = NULL) 
{
  ...
  equity <- data.frame(data %>% select(contains("equity"), 
    -contains("private"), -contains("private_debt"), -contains("alternative")))
  fixed <- data.frame(data %>% select(contains("fixed"), contains("treasury"), 
    contains("inflation_linked_bonds"), contains("corporate_bonds")))
  private.equity <- data.frame(data %>% select(contains("private_equity")))
  real.estate <- data.frame(data %>% select(contains("estate")))
  cash <- data.frame(data %>% select(contains("short"), contains("cash")))
  infrastructure <- data.frame(data %>% select(contains("infrastructure")))
  hedge <- data.frame(data %>% select(contains("hedge"), -contains("equity")))
  x <- c(colnames(equity), colnames(fixed), colnames(private.equity), 
    colnames(real.estate), colnames(cash), colnames(infrastructure), 
    colnames(hedge))
  other <- data.frame(data %>% select(-contains(x), -contains("year"), 
    -contains("plan_id"), -contains("display_name"), -contains("state"), 
    -contains("market")))
  ...
  assets <- data %>% select(year, display_name, state, private.equity, 
    real.estate, hedge, infrastructure, other.alternatives, 
    equity, fixed, cash)
  if (!is.null(plan)) {
  ...
  colnames(assets) <- c("year", "Plan", "State", "Private Equity", 
    "Real Estate", "Hedge Funds", "Infrastructure", "Other Alternatives", 
...
assets <- filterAssets(data = assets, fy = 2001, plan = "Public Employees' Retirement System of Mississippi")
colnames(assets)[2] <- "plan_name"

PERSI.assets <- data.table(assets)

#### Assigning any gap w/ 100% to Equities

#### Removing unnecessary columns
PERSI.assets <- data.frame(PERSI.assets %>% select(-Total))
data <- assets
############# assetPlot()
#View(PERSI.assets)
...
{
  data <- data.table(data)
  colnames( data) <- c("year", "Plan", "State", "Private Equity", 
    "Real Estate",  "Hedge Funds", "Infrastructure", "Other Alternatives","Total Public Equity", "Fixed Income", "Cash Equivalents")
...
assets <- assetPlot(PERSI.assets, interactive = FALSE) 
assets
```
...
ggplot(SimulationData, aes(x = Data, y = `Plan Name`, fill = `Plan Name`)) +
  geom_density_ridges(rel_min_height = 0.01,
                      scale = 3,
                      alpha = 0.7) +
  geom_vline(xintercept = FundARR, 
             linetype = "dashed",
             color = "orange",
             size = 1) +
  scale_x_continuous(breaks = seq(-0.01, 0.11, 0.02),
                     labels = percent_format(accuracy = 1),
                     limits = c(-0.05,0.15), 
                     expand = c(0, 0)) +
  labs(x = "Expected Return", y = NULL) + 
  theme_ridges() + 
...
### Current Contributions Are Actuarially Insufficient (2020-2050) w/ R (linePlot())
...
data <- as.data.table(data)
  ...
  ggplot2::ggplot(graph, ggplot2::aes(x = year, y = yaxisScale * 
    ...
    y = labelY)+
    theme(legend.direction = "vertical", legend.box = "horizontal", 
    legend.position = c(1.05, 0.14)) + labs(title = paste(title), caption = ifelse(isTRUE(caption), 
    paste("reason.org/pensions"), paste(""))) + ggplot2::theme(axis.ticks = if (isFALSE(ticks)) {
    ggplot2::element_blank()
  }
  else {
...
sensitivity <- Data %>% mutate_all(as.numeric)  %>% 
                         select(npl_pct_up, npl,npl_pct_down, mva) %>% mutate_all(as.numeric)
sensitivity <- sensitivity %>% mutate(tpl_pct_up = npl_pct_up + mva,
                               tpl = npl + mva,
                               tpl_pct_down = npl_pct_down + mva)
...
reDiscount <- function(data, sensitivity){
  ...
  sens <- sens %>% mutate(aal.duration = ((tpl_pct_down/tpl_pct_up)^(0.5)-1)*100,
...
#Use database numbers to populate/calculate needed values
table[1:4,1] <- c(seq(as.numeric(Data$dr),0.04, by = -0.01))
table[1:4,4] <- rep(Data$ee_nc_pct,4)
table[1:4,2] <- c(Data$total_nc_pct, redisc$nc.dr1, redisc$nc.dr2, redisc$nc.dr3)
table[1:4,3] <- as.numeric(table[1:4,2])-as.numeric(table[1:4,4])
table <- data.frame(table)
colnames(table) <- c("Discount Rate", 
                  "Gross Normal Cost", 
                  "Employer Normal Cost", 
...
#Format data to %/$ for appearance
table[1:4,1] <- scales::percent(table$`Discount Rate`, accuracy = 0.1)
table[1:4,2] <- scales::percent(table$`Gross Normal Cost`, accuracy = 0.1)
table[1:4,3] <- scales::percent(table$`Employer Normal Cost`, accuracy = 0.1)
table[1:4,4] <- scales::percent(table$`Employee Normal Cost`, accuracy = 0.1)
#View(table)
#Knit the table
#knitr::kable(
#  table, caption = 'Pension Integrity Project pension database.'
...
### 30-Year Employer Contribution Forecast w/ R (barPlot())

```{r stochastic, dpi=400, echo = FALSE, fig.cap = "Source: Pension Integrity Project actuarial forecast of SCRS.", fig.width=8, fig.height=5, fig.align = "center", message=FALSE, warning=FALSE}
rm(list=ls())
###Load/install packages
...
pl <- planList()
reason.data <- pullData(pl, "Idaho Public Employee Retirement System") %>% filter(year > 2000 & !duplicated(year))# (2002-2020)
  ...
  lab9 = NULL) 
{
  ...
  if (isTRUE(net.amo)) {
    ...
    reason.data$amo.payments <- if (sum(reason.data$amortization_payment_total_amount) != 
      0) {
    ...
    colnames(reason.data) <- c("year", "Interest Accrued on Unfunded Liabilities", 
      "Unfunded Liability Payments", "Negative Amo.", 
      "Positive Amo.")
    ...
    graph <- ggplot(reason.data, aes(x = V1, y = value, 
      ...
      ""), name = "", breaks = seq(2000, 2021, by = 2), 
      limits = c(2000, 2021), expand = c(0, 0)) + theme_bw() + 
      plotTheme + theme(axis.line = element_line(), panel.grid.major = element_blank(), 
    ...
    graph <- ggplot(reason.data) + geom_col(aes(x = year, 
      y = adec, fill = "Contributions Not Made"), position = "stack") + 
      ...
      fill = "Actual Contributions"), position = "stack") + 
      ...
      color = "black")) + scale_x_continuous(labels = function(x) paste0(x, 
      ""), name = "", breaks = seq(2000, 2021, by = 2), 
      limits = c(2001, 2021), expand = c(0, 0)) + theme_bw() + 
      plotTheme + theme(axis.line = element_line(), panel.grid.major = element_blank(), 
      panel.grid.minor = element_blank(), panel.border = element_blank(), 
      panel.background = element_blank()) + theme(legend.position = c(0.25, 
      0.8))
    ...
    graph <- ggplot(data_perc, aes(x = year, y = X25perc)) + 
      geom_col(aes(x = year, y = X75perc, fill = "Range of Reasonable Outcomes"), 
        alpha = stochastic.alpha) + geom_col(aes(x = year, 
      y = X25perc), fill = "white") + geom_line(aes(x = year, 
      y = X50perc, color = "Median"), size = 2) + scale_fill_manual(values = c(palette_reason$DarkBlue, 
      "white", palette_reason$Orange)) + theme(panel.grid.major.y = element_line(colour = "white", 
    ...
    graph <- ggplot(reason.data) + geom_col(aes(x = year, 
      ...
      theme_bw() + plotTheme + theme(axis.line = element_line(), 
...
### 30-Year Funded Ratio Forecast w/ R (barPlot())
...
stochasticFR <- barPlot(
  ...
  str = 25,
  stochastic = TRUE, 
  stochastic.url = "https://raw.githubusercontent.com/ReasonFoundation/databaseR/master/files/perctest2FR.csv",
  Ymax = 180,
  Ymin = 0,
  Ybreak = 10,
  yaxisScale = 1/1000000000, 
...
#Pull plan-specific data from the database
pl <- planList()
reason.data <- pullData(pl, "Public Employees' Retirement System of Mississippi") %>% filter(year > 2000 & !duplicated(year))
#View(reason.data)
#(2002-2020)
#reason.data <- pullData(pl, "Louisiana State Employees Retirement System")# (2002-2019)
#reason.data <- pullData(pl, "New Mexico Public Employees Retirement Association")# (2002-2019)
...
reason.data[,4:(length(reason.data)-1)] <- data.table(reason.data[,4:(length(reason.data)-1)]) %>% dplyr::mutate_all(as.numeric)
reason.data$year <- as.numeric(reason.data$year)
reason.data$contribution_rate_effective_fy[1:5] <- 2002:2006
reason.data$fiscal_year_of_contribution <- reason.data$contribution_rate_effective_fy

#View(reason.data$contribution_rate_effective_fy)
#### Add Net Amortization ####
#(Interest on Debt + Amortization Payments)
#Code alternative way>> 
#Amo. contribution % * Future Payroll
...
reason.data$interest_on_debt_dollar[5] <-465365041
#Calculating 2005 Interest on Debt
# reason.data$interest_on_debt_dollar[5] <- as.numeric(
# reason.data$unfunded_actuarially_accrued_liabilities_dollar[4]*0.08+
#        (reason.data$total_normal_cost_percentage[5]-reason.data$total_amortization_payment_percentage[5])*reason.data$payroll2[5]*0.0392)
# 
# 
# View(reason.data$interest_on_debt_dollar)


#Adding 2003 data manually
#reason.data$interest_on_debt_dollar[3] <- 228000000
#reason.data$amortization_payment_total_amount[3] <- -245000000
### Net Amo
net.amo <- barPlot(
  data = reason.data, 
  net.amo = TRUE, 
...
vest <- 8
vest.prob <- round(as.numeric(SalaryData2$RemainingProb[vest]),3)
vest.20 <- round(as.numeric(SalaryData2$RemainingProb[20]),3)
...
ret <- ggplot(SalaryData2)+
 geom_line(aes(Age, RemainingProb,
                group = 2,
                text = paste0("Age: ", Age,
